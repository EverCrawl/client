...
function calcProjection(viewport: Viewport, near: number, far: number): m.Matrix4 {
    return m.m4.orthographic(0, viewport.width, viewport.height, 0, near, far);
}
...
export class Camera {
    ...
    private resize = () => {
        this.projection_ = calcProjection(this.viewport, this.near_, this.far_);
    }
}
